...
func TestMatchEventFineGrainedRbacNamespaceObject(t *testing.T) {
	// Given: event and matching rbac data
	// setup event data
	eventNamespace := "event-namespace"
	eventCluster := "event-cluster"
	eventData := map[string]any{"kind": "Namespace"}

	// setup matching rbac
	fgRbacNamespaces := []string{"foo", eventNamespace}
	userData := rbac.UserData{FGRbacNamespaces: make(map[string][]string, 1)}
	userData.FGRbacNamespaces[eventCluster] = fgRbacNamespaces

	// When: we check if the rbac matches event
	match := matchEventFineGrainedRbacNamespaceObject(userData, eventData, eventNamespace, eventCluster)

	// Then: we expect a match
	assert.True(t, match, "Expected rbac to match event")
}

func TestMatchEventFineGrainedRbacNamespaceObjectNoMatch(t *testing.T) {
	// Given: event and non-matching rbac data
	// setup event data
	eventNamespace := "event-namespace"
	eventCluster := "event-cluster"
	eventData := map[string]any{"kind": "Namespace"}

	// setup non-matching rbac
	fgRbacNamespaces := []string{"foo", "bar"}
	userData := rbac.UserData{FGRbacNamespaces: make(map[string][]string, 1)}
	userData.FGRbacNamespaces[eventCluster] = fgRbacNamespaces
	...
	// Then: we expect no match
	assert.False(t, match, "Expected rbac not to match event")
}

func TestMatchEventFineGrainedRbacNamespaceObjectIsNotNamespaceObject(t *testing.T) {
	// Given: non-namespace event and rbac data
	// setup event data
	eventNamespace := "event-namespace"
	eventCluster := "event-cluster"
	eventData := map[string]any{"kind": "NotANamespace"}
	...
	// When: we check if rbac matches the event
	match := matchEventFineGrainedRbacNamespaceObject(userData, eventData, eventNamespace, eventCluster)

	// Then: there's no match because event is not namespace object
	assert.False(t, match, "Expected rbac to not match because event is not namespace object")
}

func TestMatchEventFineGrainedRbacNamespaceObjectContainsApigroupKey(t *testing.T) {
	// Given: event with apigroup and rbac data
	// setup event data
	eventNamespace := "event-namespace"
	eventCluster := "event-cluster"
	eventData := map[string]any{"kind": "Namespace", "apigroup": ""}
	...
	// Then: there's no match because event data contains apigroup
	assert.False(t, match, "Expected rbac to not match because event contains apigroup")
}

func TestMatchEventFineGrainedRbacClusterAndNamespace(t *testing.T) {
	// Given: event and rbac data
	// setup event data
	eventNamespace := "event-namespace"
	eventCluster := "event-cluster"
	...
	// When: we check if rbac matches the event
	match := matchEventFineGrainedRbacClusterAndNamespaces(userData, eventNamespace, eventCluster)

	//  Then: we expect a match
	assert.True(t, match, "Expected rbac to match event")
}

func TestMatchEventFineGrainedRbacClusterAndNamespaceAllNamespaces(t *testing.T) {
	// Given: event and rbac data
	// setup event data
	eventNamespace := "event-namespace"
	eventCluster := "event-cluster"
...
func TestMatchEventFineGrainedRbacClusterAndNamespaceNoMatch(t *testing.T) {
	// Given: event and non-matching rbac data
	// setup event data
	eventNamespace := "event-namespace"
	eventCluster := "event-cluster"
	...
	// setup rbac data
	fgRbacNamespaces := []string{"that-namespace", "some-other-namespace"}
	userData := rbac.UserData{FGRbacNamespaces: make(map[string][]string, 1)}
	userData.FGRbacNamespaces[eventCluster] = fgRbacNamespaces
	...
	//  Then: there's no match because namespace doesn't match
	assert.False(t, match, "Expected rbac not to match event")
}

func TestMatchEventFineGrainedRbacGroupKind(t *testing.T) {
	// Given: event data
	// setup event data
	eventKind := "VirtualMachine"
	eventApigroup := "kubevirt.io"

	// When: we check if event data matches fine-grained rbac apigroup kind list
	match := matchEventFineGrainedRbacGroupKind(eventApigroup, eventKind)

	// Then: there's a match
	assert.True(t, match, "Expected event apigroup and kind to match")
}

func TestMatchEventFineGrainedRbacGroupKindNoMatch(t *testing.T) {
	// Given: event data
	// setup event data that doesn't match
	eventKind := "asdf"
	eventApigroup := "kubevirt.io"
	...
	// Then: there's no match because eventKind isn't valid
	assert.False(t, match, "Expected eventKind %s and apigroup not to match", eventKind)
}

func TestMatchEventManagedClusterRbac(t *testing.T) {
	// Given: event and rbac data
	// setup event data
	eventCluster := "event-managed-cluster"
	eventData := map[string]any{"name": "foo", "cluster": eventCluster}

	// setup rbac data
	userData := rbac.UserData{ManagedClusters: make(map[string]struct{}, 1)}
	userData.ManagedClusters[eventCluster] = struct{}{}

	// When: we check if managed cluster rbac matches event data
	match := matchEventManagedClusterRbac(userData, eventData, eventCluster)

	// Then: there's a match
	assert.True(t, match, "Expected managed clusters rbac to match event cluster")
}

func TestMatchEventManagedClusterRbacNoMatch(t *testing.T) {
	// Given: event data that doesn't match rbac
	// setup event data
	eventCluster := "event-managed-cluster"
	eventData := map[string]any{"name": "foo", "cluster": eventCluster}
	...
	// setup rbac data
	userData := rbac.UserData{ManagedClusters: make(map[string]struct{}, 1)}
	userData.ManagedClusters["adsf"] = struct{}{}

	// When: we check if managed cluster rbac matches event data
	match := matchEventManagedClusterRbac(userData, eventData, eventCluster)

	// Then: there's no match
	assert.False(t, match, "Expected managed clusters rbac not to match event cluster")
}

func TestMatchEventManagedClusterRbacHubClusterResource(t *testing.T) {
	// Given: hub cluster event that doesn't match managed cluster rbac
	// setup event data
	eventCluster := "event-managed-cluster"
	eventData := map[string]any{"name": "foo", "cluster": eventCluster, "_hubClusterResource": true}

	// setup rbac data
	userData := rbac.UserData{ManagedClusters: make(map[string]struct{}, 1)}
	userData.ManagedClusters[eventCluster] = struct{}{}

	// When: we check if managed cluster rbac matches hub cluster event data
	match := matchEventManagedClusterRbac(userData, eventData, eventCluster)

	// Then: there's no match
	assert.False(t, match, "Expected managed clusters rbac not to match hub cluster event")
}

func TestMatchEventClusterScopedResourcesNoResources(t *testing.T) {
	// Given: cluster-scoped event and user rbac
	// setup event data
	...
	// setup rbac data
	userData := rbac.UserData{CsResources: make([]rbac.Resource, 0)}
...
func TestMatchEventClusterScopedResourcesMatch(t *testing.T) {
	...
	// setup rbac data
	resourcePerms := []rbac.Resource{rbac.Resource{Kind: eventKind, Apigroup: eventApigroup}}
	userData := rbac.UserData{CsResources: resourcePerms}

	// When: we check if cluster-scoped resources matches user rbac
	match := matchEventClusterScopedResources(userData, eventKind, eventApigroup, eventNamespace)

	// Then: there's a match
	assert.True(t, match, "Expected cluster-scoped resource list to match")
}

func TestMatchEventClusterScopedResourceNoMatchHasNamespace(t *testing.T) {
	// Given: cluster-scoped event with namespace and user rbac
	// setup event data
	eventKind := "event-kind"
	eventApigroup := "event-apigroup"
	eventNamespace := "event-namesepace"

	// setup rbac data
	resourcePerms := []rbac.Resource{rbac.Resource{Kind: "foo", Apigroup: "bar"}}
	userData := rbac.UserData{CsResources: resourcePerms}

	// When: we check if cluster-scoped resources match user rbac
	match := matchEventClusterScopedResources(userData, eventKind, eventApigroup, eventNamespace)

	// Then: there's no match because namespace != "" and rbac doesn't match
	assert.False(t, match, "Expected cluster-scoped resource list not to match because namespace != \"\"")
}

func TestMatchEventClusterScopedResourceAllKindApigroup(t *testing.T) {
	// Given: cluster-scoped event and user rbac with access to all apigroup and kind
	// setup event data
	eventKind := "event-kind"
	eventApigroup := "event-apigroup"
	eventNamespace := "event-namespace"
	...
	// setup rbac data
	resourcePerms := []rbac.Resource{rbac.Resource{Kind: "*", Apigroup: "*"}}
	userData := rbac.UserData{CsResources: resourcePerms}
	...
	// Then: there's a match
	assert.True(t, match, "Expected cluster-scoped resource list to match * apigroup and * kind")
}

func TestMatchEventApigroupKindAllApiGroupAllKind(t *testing.T) {
	// Given: event data and matching cluster scoped rbac data
	// setup event data
	eventKind := "VirtualMachine"
	eventApigroup := "kubevirt.io"

	// setup rbac data
	userData := rbac.UserData{CsResources: make([]rbac.Resource, 1)}
	resourcePerm := rbac.Resource{Kind: "*", Apigroup: "*"}
	userData.CsResources[0] = resourcePerm

	// When: we check if the event matches rbac data
	match := matchEventApigroupKind(userData, eventKind, eventApigroup)

	// Then: there's a match
	assert.True(t, match, "Expected apigroup * and kind * to match")
}

func TestMatchEventApigroupKindAllApiGroupCertainKind(t *testing.T) {
	// Given: event data and matching cluster scoped rbac data
	// setup event data
	eventKind := "VirtualMachine"
	eventApigroup := "kubevirt.io"
	...
	// setup rbac data
	userData := rbac.UserData{CsResources: make([]rbac.Resource, 1)}
	resourcePerm := rbac.Resource{Kind: eventKind, Apigroup: "*"}
	userData.CsResources[0] = resourcePerm
...
func TestMatchEventApigroupKindCertainApiGroupAllKind(t *testing.T) {
	// Given: event data and matching cluster scoped rbac data
	// setup event data
	eventKind := "VirtualMachine"
	eventApigroup := "kubevirt.io"
	...
	// setup rbac data
	userData := rbac.UserData{CsResources: make([]rbac.Resource, 1)}
	resourcePerm := rbac.Resource{Kind: "*", Apigroup: eventApigroup}
	userData.CsResources[0] = resourcePerm
...
func TestMatchEventApigroupKindCertainApiGroupCertainKind(t *testing.T) {
	// Given: event data and matching cluster scoped rbac data
	// setup event data
	eventKind := "VirtualMachine"
	eventApigroup := "kubevirt.io"
	...
	// setup rbac data
	userData := rbac.UserData{CsResources: make([]rbac.Resource, 1)}
	resourcePerm := rbac.Resource{Kind: eventKind, Apigroup: eventApigroup}
	userData.CsResources[0] = resourcePerm
...
func TestMatchEventApigroupKindNoMatch(t *testing.T) {
	// Given: event data and non-matching cluster scoped rbac data
	// setup event data
	eventKind := "VirtualMachine"
	eventApigroup := "kubevirt.io"
	...
	// setup rbac data
	userData := rbac.UserData{CsResources: make([]rbac.Resource, 1)}
	resourcePerm := rbac.Resource{Kind: "foo", Apigroup: "bar"}
	userData.CsResources[0] = resourcePerm
	...
	// Then: there's no match
	assert.False(t, match, "Expected apigroup and kind not to match")
}

func TestMatchEventNamespaceScopedResources(t *testing.T) {
	// Given: event namespace and namespace-scoped rbac data
	// setup event data
	eventNamespace := "event-namespace"

	// setup rbac data
	userData := rbac.UserData{NsResources: make(map[string][]rbac.Resource, 1)}
	userData.NsResources[eventNamespace] = []rbac.Resource{}

	// When: we check if the event matches rbac data
	match := matchEventNamespaceScopedResources(userData, eventNamespace)

	// Then: there's a match
	assert.True(t, match, "Expected event namespace to match rbac namespace")
}

func TestMatchEventNamespaceScopedResourcesNoNamespacePermission(t *testing.T) {
	// Given: event namespace and empty namespace-scoped rbac data
	// setup event data
	eventNamespace := "event-namespace"

	// setup rbac data
	userData := rbac.UserData{NsResources: nil}

	// When: we check if the event matches rbac data
	match := matchEventNamespaceScopedResources(userData, eventNamespace)

	// Then: there's no match
	assert.False(t, match, "Expected event namespace not to match empty rbac namespaces")
}

func TestMatchEventNamespaceScopedResourcesAllNamespaces(t *testing.T) {
	// Given: event namespace and * namespace-scoped rbac data
	// setup event data
	eventNamespace := "event-namespace"
	...
	// setup rbac data
	userData := rbac.UserData{NsResources: make(map[string][]rbac.Resource, 1)}
	userData.NsResources["*"] = []rbac.Resource{}
...
func TestMatchEventNamespaceScopedResourcesNoMatch(t *testing.T) {
	// Given: event namespace and non-matching namespace-scoped rbac data
	// setup event data
	eventNamespace := "event-namespace"

	// setup rbac data
	userData := rbac.UserData{NsResources: make(map[string][]rbac.Resource, 1)}
	userData.NsResources["asdf"] = []rbac.Resource{}

	// When: we check if the event matches rbac data
	match := matchEventNamespaceScopedResources(userData, eventNamespace)

	// Then: there's no match
	assert.False(t, match, "Expected event namespace to not match rbac namespace")
}

func TestMatchEventHubClusterRbacNoClusterNorNamespaceResources(t *testing.T) {
	// Given: event data and rbac without namespace nor cluster-scoped resources
	// setup event data
	eventData := make(map[string]any, 0)
	eventNamespace := ""
	eventKind := ""
	eventApigroup := ""

	// setup rbac data
	userData := rbac.UserData{CsResources: nil, NsResources: nil}

	// When: we check if the event matches rbac
	match := matchEventHubClusterRbac(userData, eventData, eventNamespace, eventKind, eventApigroup)

	// Then: there's no match
	assert.False(t, match, "Expected event not to match empty rbac")
}

func TestMatchEventHubClusterRbacNotHubClusterEvent(t *testing.T) {
	// Given: event data that's not a hub resource and user rbac
	// setup event data
	eventData := make(map[string]any, 0)
	eventNamespace := ""
	eventKind := ""
	...
	// setup rbac data
	userData := rbac.UserData{CsResources: make([]rbac.Resource, 1), NsResources: make(map[string][]rbac.Resource, 1)}
	userData.CsResources[0] = rbac.Resource{Kind: "*", Apigroup: "*"}
	userData.NsResources["adsf"] = []rbac.Resource{rbac.Resource{Kind: "*", Apigroup: "*"}}
